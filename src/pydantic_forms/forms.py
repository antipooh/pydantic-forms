...
class PydanticForm(BaseModel, Generic[T]):
    ...
    csrf:
        ...
        * :func:`create() <PydanticForm.create>` creates the attribute to allow the user to embed it in hidden fields
            on the client.
        * :func:`validate_request() <PydanticForm.validate_request>` will have the results of the form data

        Pydantic-Forms expects and requires you to embed this token on each one of your HTML forms whether as a hidden
            field or otherwise.
    ...
    model:
        ...
        * :func:`validate_request <Pydantic.validate_request` will have the model as the pydantic model if validation
    ...
    async def validate_request(cls, request: Any, schema: Type[BaseModel],
        ...
        **If the form data caused a ValidationError**
        ...
        assert csrf_form_data is not None
        return cls(fields=form_fields, model=model, csrf=csrf_form_data)
